...
async function groupBranchesByPRStatus(octokit, repoInfo, branches) {
  ...
  try {
    // Fetch all PRs once instead of searching for each branch individually
    console.log('📡 Fetching all PRs for repository...');
    const allPRs = await githubApi.getAllPullRequests(
      octokit,
      repoInfo.owner,
      repoInfo.name
    ...
    console.log(`✅ Found ${allPRs.length} PRs total, processing ${branches.length} branches...`);

    // Now process each branch with local lookup (no API calls)
    for (const branch of branches) {
      try {
        const pr = branchToPRMap.get(branch);
...
async function findBranchesToDelete(octokit, repoInfo, branches, options, currentBranch) {
  ...
  try {
    ...
    // Process each branch with local lookup (no API calls)
    for (const branch of branches) {
      try {
        const pr = branchToPRMap.get(branch);
